...
public class EarthWandEntity extends AbstractArrow implements ItemSupplier {
	...
	public static EarthWandEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
	...
	public static EarthWandEntity shoot(LivingEntity entity, LivingEntity target) {
		...
		entity.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.rooted_dirt.fall")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
