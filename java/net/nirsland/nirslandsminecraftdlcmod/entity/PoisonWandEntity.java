...
public class PoisonWandEntity extends AbstractArrow implements ItemSupplier {
	...
	public static PoisonWandEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
	...
	public static PoisonWandEntity shoot(LivingEntity entity, LivingEntity target) {
		...
		entity.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.slime_block.place")), SoundSource.PLAYERS, 1, 1f / (new Random().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
